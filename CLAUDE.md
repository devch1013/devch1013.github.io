...
**The form is chosen per entry — there is no house widget.** Start from one sentence naming what the reader must understand from the picture, and let that sentence's shape pick the form: a layered view for surface-vs-reality, a recalculating panel when one value decides the outcome, a branch map for diverging paths, nested boxes for containment, inline SVG for spatial relationships, a matrix when the answer varies by combination, a playable timeline only when *time* is genuinely part of the insight. That list is examples, not a menu — invent the form the content asks for. Animation and interaction are options, never requirements; a still picture that explains better is the right answer.

Before building, write down a three-line design plan — four to six colors drawn from the subject and used to encode meaning (not decoration), which roles get a mono face (inherit the site body font; never load an external one), and one sentence of layout — then ask whether that plan could be pasted onto any other entry. If it could, it isn't this entry's plan.

**Read `_includes/codex/` first and make sure the new one doesn't share a skeleton with any of them.** Same controls, same stage, same eye path reads as the same picture no matter how different the topic is. The play/pause/reset + segmented-axis + lane-timeline + counter + verdict skeleton is off limits for now — every existing entry was built from it, which is exactly the problem.

Two things hold regardless of form: include the failing case, since showing only the happy path removes the contrast that teaches; and if the visual computes anything, gather that computation into one pure function so it can be verified headlessly, driving the view from its output rather than from a live state machine. An interaction has to produce new information — swapping between pre-written panels is a folded-up table, so write the table instead.

Rules: no `iframe` (breaks auto-height, TOC, search and print — an include has none of those problems); scope all CSS under one wrapper class, never bare element or generic selectors, since the entry's stylesheet is the theme's; vanilla JS only, inline in the same file, wrapped in an IIFE and rooted via `document.currentScript.closest(...)` because a page may hold several includes; use native controls (`<button type="button">`, `<input type="range">`) with `aria-pressed` so keyboard access comes free; honor `prefers-reduced-motion` by jumping to the final state rather than refusing to run; and keep the entry readable with JS off — add a `js` class from the script and let CSS fall back to a static summary table, with anything only the widget says also appearing as a sentence in the prose.
