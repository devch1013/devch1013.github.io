...
**Interactive sections go in `_includes/codex/<slug>.html`,** pulled in with `{% include codex/<slug>.html %}`. Reserve them for cases where changing a value is what makes the mechanism click (scheduling order, cache hit/miss, concurrency timing, pool exhaustion).

Build them as **simulators, not toggles.** A control that swaps between pre-written result panels is a folded-up table — write the table instead. What earns an include is state that *moves*: play/pause/reset controls, one or two condition axes, a stage where blocks, slots or queue items fill and advance over ticks, live counters that update as it runs (and visibly stall on the broken combination), and a verdict line underneath. Pick one quantity that visibly changes between combinations — a marker sliding later, a pool filling to its cap, a counter freezing — and build the whole thing around making that legible. Always include a failing combination; showing only the happy path removes the contrast that teaches. Implement it deterministically: precompute a per-combination schedule and let a playhead reveal it, rather than running a live state machine — it is shorter and it is the only form that can be verified.
