...
애플리케이션 인스턴스를 여러 대 띄워 놓으면 이 버그의 성질이 한 번 더 바뀐다. **인스턴스 1 이 남긴 세션 상태가 인스턴스 2·3 의 트랜잭션을 죽인다.** 프로세스가 다르고 메모리를 한 톨도 공유하지 않는데도 그렇다.
...
그리고 같은 풀러를 공유하는 것이 내 인스턴스들만이라는 보장도 없다. 그 층위 — 남의 애플리케이션이 남긴 오염 — 는 아래 「이 해법의 한계」에서 이어서 다룬다.
