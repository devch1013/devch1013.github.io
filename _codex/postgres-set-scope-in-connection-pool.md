...
아래 그림 넉 장이 이 글의 요약이다. **BEGIN 의 복사 화살표가 무엇을 실어 내리는지**만 세로로 훑으면 된다 — 트랜잭션의 읽기전용 플래그는 그 순간 세션 기본값을 한 번 복사해 오고 그걸로 끝이라, 복사가 지나간 뒤에 기본값을 바꿔봐야 이번 트랜잭션은 꿈쩍도 하지 않는다.
